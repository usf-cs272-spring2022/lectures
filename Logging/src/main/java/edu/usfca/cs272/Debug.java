...
/**
 * This class demonstrates a basic debug class, used to motivate using a logging
 * framework instead.
 *
...
public class Debug {
	 ...
	 * the console. Otherwise, no output is produced.
	 *
	 * @param message to output when debugging is enabled
	 */
	 ...
	 * messages for just one class. If this class is widely used, turning on debug
	 * messages could result in a lot of unnecessary output. We could try to have
	 * a per-class debug flag, but then we run into other issues.
	 *
	 * We may also want some debug messages within a class to output, but not all.
	 * For example, we may still want to see error messages. We could try to add
	 * multiple levels of debug statements, but then our class starts to get more
	 * and more complicated and computationally expensive.
	 *
	 ...
	 * error occurs. Again, we can try to achieve this in this class, but we have
	 ...
	 *
	 * In some cases, more messages than the console can "remember" will be added,
	 * causing the early debug messages to be lost.
	 *
	 * In multithreading contexts, need the timestamp of when the message was
